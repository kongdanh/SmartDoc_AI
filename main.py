...
FIXES applied:
  1. CompareRequest imported from models (was missing → 404 on /api/compare-rag)
  2. /chat: session_id not found → auto-create new session instead of 404
  3. Standard RAG ChromaDB race condition → asyncio.Lock() prevents concurrent resets
  4. /chat/sessions DELETE all → added missing delete_all() call
"""
...
# Windows: ProactorEventLoopPolicy for subprocess support
...
from source.models import (
    ChatRequest,
    CompareRequest,          # FIX #1: was missing → caused /api/compare-rag to fail with ImportError
    DirectSearchRequest,
...
# FIX #3: Global lock prevents two concurrent Standard RAG index builds
# from racing on ChromaDB collection creation/deletion
...
async def _background_index():
    try:
        # [FIX] Tải sẵn model embedding vào RAM lúc khởi động server 
        # để câu hỏi đầu tiên không bị đứng 12 giây
        logger.info("Pre-loading Embedding Model into memory...")
        from standard_rag import _get_embedder
        ...
        if results:
            ...
            logger.info(
                "No domains found to index. Add subfolders to %s",
                settings.data_path,
            )
    except Exception:
...
async def _safe_build_standard_rag(domain: str):
    """Build Standard RAG index with lock to prevent ChromaDB race conditions."""
    async with _rag_index_lock:
        await asyncio.to_thread(build_standard_rag_index, domain)
...
async def search_direct(req: DirectSearchRequest):
    result = await query_direct(domain=req.domain, query=req.query, top_k=req.top_k)
    if result.get("error"):
        raise HTTPException(status_code=400, detail=result["error"])
...
async def chat(req: ChatRequest):
    """
    Send a chat message, receive a streaming SSE response.

    FIX #2: If session_id is provided but session no longer exists
    (e.g. server restarted), auto-create a new session instead of
    returning 404, which caused the UI to break silently.
    """
    session = None

    if req.session_id:
        session = sessions.get(req.session_id)
        # FIX #2: stale session_id → create fresh session, don't 404
        if session is None:
            logger.warning(
                "session_id '%s' not found — creating new session for domain '%s'",
                req.session_id,
                req.domain,
            )
...
async def delete_all_chat_sessions():
    """FIX #4: delete_all() was calling undefined method — use list+delete loop."""
    all_ids = [s.id for s in sessions.list_all()]
    for sid in all_ids:
        sessions.delete(sid)
    return {"message": f"{len(all_ids)} sessions deleted"}
...
async def reindex_domain(domain: str):
    ...
    if not data_dir.is_dir():
        raise HTTPException(
            status_code=404,
            detail=f"Domain '{domain}' not found in data directory",
        )
    ...
    # FIX #3: use locked wrapper so Standard RAG doesn't race with itself
    asyncio.create_task(_safe_build_standard_rag(domain))

    return ReindexResponse(
        message=f"Re-indexing started for domain '{domain}'",
        domains=[domain],
    )
...
async def reindex_all():
    ...
    # FIX #3: sequential locked builds to avoid ChromaDB conflicts
    async def _rebuild_all_standard():
        for d in domains:
            await _safe_build_standard_rag(d)

    asyncio.create_task(_rebuild_all_standard())

    return ReindexResponse(
        message="Re-indexing started for all domains",
        domains=domains,
    )
...
async def upload_files(domain: str, files: list[UploadFile]):
    """Upload files to a domain's data directory and trigger Standard RAG indexing."""
    if not re.match(r'^[a-zA-Z0-9_\-]+$', domain):
        raise HTTPException(
            status_code=400,
            detail="Domain name must contain only letters, numbers, underscores, or hyphens",
        )
    ...
    for f in files:
        ...
        # Organised uploads storage
        upload_type_dir = settings.upload_path / (
            "pdf" if suffix == ".pdf"
            else "docx" if suffix in (".docx", ".doc")
            else "txt"
        )
        upload_type_dir.mkdir(parents=True, exist_ok=True)
        (upload_type_dir / safe_name).write_bytes(content)

        # Convert to TXT for GraphRAG
        if suffix == ".pdf":
            convert_pdf_to_txt(dest, dest.with_suffix(".txt"))
        elif suffix in (".docx", ".doc"):
            convert_docx_to_txt(dest, dest.with_suffix(".txt"))

        uploaded += 1

    if uploaded > 0:
        # FIX #3: use locked wrapper
        asyncio.create_task(_safe_build_standard_rag(domain))
...
async def delete_domain_file(domain: str, filename: str):
    ...
    # Remove companion .txt if original was pdf/docx
...
async def delete_domain(domain: str):
    ...
    FileRegistry(settings.index_path / "file_registry.json").clear_domain(domain)
...
async def compare_rag(req: CompareRequest):
    """Run Standard RAG and GraphRAG in parallel and return both answers."""
